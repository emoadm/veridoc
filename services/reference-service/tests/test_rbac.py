...
def test_nonpermitted_role_gets_403(client, make_token):
    ...
    token = make_token(
        sub="ra", roles=["regulatory-affairs"], site="site-001", study="study-A"
    )
    resp = client.post(
...
def test_cross_tenant_update_is_denied(client, make_token):
    ...
    token_a = make_token(
        sub="coord-a", roles=["data-manager"], site="site-001", study="study-A"
    )
    created = client.post(
    ...
    token_b = make_token(
        sub="coord-b", roles=["data-manager"], site="site-002", study="study-B"
    )
    resp = client.put(
        f"/subjects/{subject_id}", json={"pii": "Hacked"}, headers=_auth(token_b)
    )
    assert resp.status_code == 403, resp.text
...
def test_missing_mfa_is_rejected(client, make_token):
    ...
    resp = client.post(
        "/subjects", json={"natural_id": "MRN-4", "pii": "P"}, headers=_auth(token)
    )
